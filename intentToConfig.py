...
with open("apres.json", "r") as file:
...
def generate_router_config(router, as_data, is_ebgp):
    ...
    config.append("!")
    config.append("!")
    config.append(f"! Last configuration change at 11:32:58 UTC Wed Jan 15 2025")
    config.append("!")
...
for as_data in network_data["AS"]:
    for router in as_data["routers"]:
        ...
        file_path = os.path.join(output_directory, f"{router['hostname']}.cfg")
